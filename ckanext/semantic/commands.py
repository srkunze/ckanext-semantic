...
class SemanticCommand(cli.CkanCommand):
    ...
    def update_dataset_statistics(self, dataset_name):
        ...
        dataset = model.Session.query(model.Package).filter(model.Package.name == dataset_name).one()
        dataset_statistics.set_dataset(dataset)
        dataset_statistics.set_waiting_time(pylons.config.get('ckan.semantic.waiting_time'))
        dataset_statistics.set_ratio_old_new(pylons.config.get('ckan.semantic.ratio_old_new'))
